...
const {getQueryObjPreviouslyScrapedIds, sleep} = require('./utilities');
...
Query.prototype.getJobs = async function() {
	const previouslyScrapedIds = getQueryObjPreviouslyScrapedIds(this.title);
	...
	return new Promise((resolve, reject) => {
		...
		async function getSomeJobs(self, jobs) {
			...
			request(self.url(), (error, response, body) => {
				...
				const done = pageIds.some(id => previouslyScrapedIds.includes(id));
				if (parsed.error !== null) {
					// Got an error so reject
					reject(Error);
				} else if (parsed.continue === true && !done) {
